...
public class BookEndpointTest {
	...
	@Test
	@InSequence(2)
	//injecting the web target to run this as a client
	//so we have a web target object pointing at /api/books
	...
	public void shouldCreateABook(@ArquillianResteasyResource("api/books") WebTarget webTarget) {
		...
		Book book = new Book(null, "a title", 12F, 123, Language.ENGLISH, new Date(), "imageURL", "description");
		// posts the book object using POST method, and also converting the book object into an entity here, send as JSON
		response = webTarget.request(APPLICATION_JSON).post(Entity.entity(book, APPLICATION_JSON));
		// should get 'book created' status code
	...
	public void shouldFailCreatingABookWithLowUnitCostTitle(
			@ArquillianResteasyResource("api/books") WebTarget webTarget) {
